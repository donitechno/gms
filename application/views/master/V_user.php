			...
			<div class="ui stackable menu" style="border-radius:0">
				<div class="item">
					<div class="ui breadcrumb">
					  ...
					  <div class="active section">Data User</div>
					</div>
				</div>
...
<script>
	...
	function filterTrans(){
		...
		$.ajax({
			url : webUrl+'/index.php/C_user/get_all_karyawan',
			type: "GET",
			dataType: "JSON",
	...
	function addForm(){
		...
		$.ajax({
			url : webUrl+'/index.php/C_user/get_input_form',
			type: "GET",
			dataType: "JSON",
			success: function(response){
				if(response.success == true){
	...
	function editForm(idData){
		...
		$.ajax({
			url : webUrl+'/index.php/C_user/get_edit_form/'+idData,
			type: "GET",
			dataType: "JSON",
			success: function(response){
				if(response.success == true){
	...
	function changeStatus(id,status){
		...
		var webUrl = "<?php echo base_url()?>";
		var view= '<div class="header">Ubah Status</div><div class="content"><p>Anda Ingin Mengubah Status Karyawan Menjadi '+pesanStatus+'?</p></div><div class="actions"><div class="ui negative button">Tidak </div><button id="btn_confirm" class="ui green right labeled icon button" onclick=exeChangeStatus("'+id+'","'+status+'")>Ya<i class="checkmark icon"></i></button></div>';
	...
	function exeChangeStatus(id,status){
		if(exeTrans == false){
			...
			$.ajax({
				url : webUrl+'/index.php/C_user/change_status/'+id+'/'+status,
				type: "GET",
				dataType: "JSON",
				success: function(response){
					if(response.success == false){
