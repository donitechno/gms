...
class M_product extends CI_Model {
	...
	public function get_posisi_detail_pajangan($per_tanggal,$filter_box_from,$filter_box_to,$filter_pindah_box){
		$sql = "SELECT s.*, k.karat_name, c.category_name, f.from_name
				...
				WHERE (s.id_box BETWEEN '$filter_box_from' AND '$filter_box_to' AND s.in_date < '$per_tanggal' AND s.out_date = '0000-00-00 00:00:00') OR (s.in_date < '$per_tanggal' AND s.out_date > '$per_tanggal') OR (s.in_date < '$per_tanggal' AND s.id IN($filter_pindah_box)) ORDER BY s.id";
	...
	public function insert_product($product_id,$old_id,$select_karat,$select_box,$select_category,$select_from,$select_from_desc,$nama_barang,$berat_barang,$tgl_stock_in,$created_by){
		$sql = "INSERT INTO gold_product(id,id_lama,id_karat,id_box,id_category,id_from,product_from_desc,product_name,product_weight,in_date,created_date,created_by) VALUES ('$product_id','$old_id','$select_karat','$select_box','$select_category','$select_from','$select_from_desc','$nama_barang','$berat_barang','$tgl_stock_in',CURRENT_TIMESTAMP(),'$created_by')";
	...
	public function update_product_stock_out($product_id,$tgl_stock_out){
		$sql = "UPDATE gold_product
				SET out_date = '$tgl_stock_out', status = 'O'
				WHERE id = '$product_id'";
	...
	public function get_product_by_id($product_id){
		$sql = "SELECT p.*, k.karat_name 
				FROM gold_product p, gold_karat k
				WHERE p.id_karat = k.id AND p.id LIKE '%$product_id'";
	...
	public function get_product_date_buy($date_data,$product_id){
		$sql = "SELECT s.*, k.karat_name, c.category_name, f.from_name
				FROM gold_product s, gold_karat k, gold_product_category c, gold_product_from f
				WHERE s.id_karat = k.id AND s.id_category = c.id AND s.id_from = f.id
				AND s.in_date < '$date_data' AND s.out_date < '$date_data' AND s.status = 'S' AND s.id LIKE '%$product_id%'
				ORDER BY s.in_date, s.created_date";
	...
	public function get_product_date_buy_2($date_data,$product_id,$filter_product){
		$sql = "SELECT s.*, k.karat_name, c.category_name, f.from_name
				FROM gold_product s, gold_karat k, gold_product_category c, gold_product_from f
				WHERE s.id_karat = k.id AND s.id_category = c.id AND s.id_from = f.id
				AND s.in_date < '$date_data' AND s.out_date < '$date_data' AND s.status = 'S' AND s.id LIKE '%$product_id%' AND s.id NOT IN($filter_product)
				ORDER BY s.in_date, s.created_date";
	...
	public function get_product_date_buy_val($date_data,$product_id){
		$sql = "SELECT s.*, k.karat_name, c.category_name, f.from_name
				FROM gold_product s, gold_karat k, gold_product_category c, gold_product_from f
				WHERE s.id_karat = k.id AND s.id_category = c.id AND s.id_from = f.id
				AND s.in_date < '$date_data' AND s.out_date < '$date_data' AND s.status = 'S' AND s.id = '$product_id'
				ORDER BY s.in_date, s.created_date";
	...
	public function get_product_date_buy_val_2($date_data,$product_id,$filter_product){
		$sql = "SELECT s.*, k.karat_name, c.category_name, f.from_name
				FROM gold_product s, gold_karat k, gold_product_category c, gold_product_from f
				WHERE s.id_karat = k.id AND s.id_category = c.id AND s.id_from = f.id
				AND s.in_date < '$date_data' AND s.out_date < '$date_data' AND s.status = 'S' AND s.id = '$product_id' AND s.id NOT IN($filter_product)
				ORDER BY s.in_date, s.created_date";
	...
	public function get_rekap_stock_out($tgl_transaksi){
		$sql = "SELECT COUNT(s.id_karat) as pcs , SUM(p.product_weight) as berat , k.karat_name, s.id_karat
				FROM gold_stock_out s, gold_karat k, gold_product p
				WHERE s.id_karat = k.id AND s.id_product = p.id AND s.trans_date = '$tgl_transaksi'
				GROUP BY s.id_karat
				ORDER BY s.id_karat";
	...
	public function unlock_product_control($id_product,$alasan_unlock,$created_by){
		$sql = "UPDATE gold_product
				SET lock_status = 'N', unlock_reason = '$alasan_unlock', unlock_by = '$created_by', unlock_date = CURRENT_TIMESTAMP()
				WHERE id = '$id_product'";
