...
class C_jual extends CI_Controller {	
	...
	private function validate_jual($row_number){
		...
		if($no_telp == ''){
			$data['success'] = FALSE;
			$data['inputerror'] .= '<li>Nomor Telepon Harus Diisi!</li>';
		}
		
		if($alamat_cust == ''){
			$data['success'] = FALSE;
			$data['inputerror'] .= '<li>Alamat Harus Diisi!</li>';
		}
	...
	public function test(){
		...
		$connector = new Escpos\PrintConnectors\WindowsPrintConnector("smb://".$computer_name."/".$printer_name);
		$printer = new Escpos\Printer($connector);
		$printer -> text("ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZ");
		$printer -> close();
	}
}