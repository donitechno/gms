...
class C_beli extends CI_Controller {	
	...
	public function find_customer(){
		...
		if($customer_phone != ''){
			$data_customer = $this->mt->get_customer_by_phone($customer_phone);
	...
	public function save_beli($row_number,$print_flag){
		...
		if($print_flag == 'P'){
			...
			$printer -> text("PENJUAL: ".strtoupper(substr($cust_name, 0, 11))."\n");
			$printer -> text("\n");
			...
			/*
			$printer -> text("                                                ".strtoupper($cust_name));
			
			$ket_length = 13;
			$cs_length = strlen($cust_name);
			...
			$printer -> text("        ".strtoupper(substr($cust_service, 0, 11))."\n");
	...
	private function validate_beli(){
		$data = array();
