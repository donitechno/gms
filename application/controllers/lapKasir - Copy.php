...
class LapKasir extends CI_Controller {
	...
	public function filter_kas(){
		...
		if($detail_rekap == 'D'){
			...
			$data['view'] = '<div class="ui grid"><div class="sixteen wide centered column full-print" style="padding-top:0px;padding-bottom:0px;text-align:right"><a class="ui facebook button" href="'.base_url().'index.php/lapKasir/lap_kasir_to_pdf/'.$tanggal_transaksi.'/'.$jenis_bayar_link.'/'.$detail_rekap.'" target=_blank"><i class="paperclip icon"></i> Download</a></div><div class="sixteen wide centered column center aligned" style="font-weight:600"><span>Laporan Transaksi Harian Rekap</span><br><span>Tanggal '.$tanggal_transaksi.', Cabang '.$site_name.'</span><br></div><div class="sixteen wide centered column">';
		...
		if($detail_rekap == 'D'){
			...
			for($i = 0;$i < $length; $i++){
				...
				if($i == 0){
					...
					$objPHPExcel->getActiveSheet()->getStyle('C'.$baris.':C'.$baris.'')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('E'.$baris.':E'.$baris.'')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		...
		if($detail_rekap == 'D'){
			...
			for($i = 0;$i < $length; $i++){
				...
				if($i == 0){
					...
					$objPHPExcel->getActiveSheet()->getStyle('G'.$baris.':G'.$baris.'')->getNumberFormat()->setFormatCode('_(* #,##0.00_);_(* (#,##0.00);_(* "-"_);_(@_)');
					$objPHPExcel->getActiveSheet()->getStyle('H'.$baris.':H'.$baris.'')->getNumberFormat()->setFormatCode('_(* #,##0_);_(* (#,##0);_(* "-"_);_(@_)');
					$objPHPExcel->getActiveSheet()->getStyle('D'.$baris.':D'.$baris.'')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
