...
class LapAccountGr extends CI_Controller{
	...
	public function index(){
		$account = $this->mm->get_coa_mas_content();
		$karat = $this->mk->get_karat_srt();
		
		$data['view'] = '<div class="ui container fluid">
			<form class="ui form form-javascript" id="lapAccountGr-form-filter" action="'.base_url().'index.php/lapAccountGr/filter" method="post">
			<div class="ui grid">
				<div class="ui inverted dimmer" id="lapAccountGr-loaderlist">
				...
				<div class="fifteen wide centered column" style="margin-top:15px">
					<div class="fields">
						<div class="three wide field">
							...
							<input type="text" name="lapAccountGr-datefrom" id="lapAccountGr-datefrom" readonly>
						</div>
						...
						<div class="three wide field">
							...
							<input type="text" name="lapAccountGr-dateto" id="lapAccountGr-dateto" readonly>
						</div>
						<div class="five wide field">
							<label>Account</label>
							<select name="lapAccountGr-accountnumber" id="lapAccountGr-accountnumber">';
						...
						<div class="five wide field">
							<label>Karat</label>
							<select name="lapAccountGr-idkarat" id="lapAccountGr-idkarat">
						...
						<div class="three wide field">
							<label style="visibility:hidden">-</label>
							<select name="lapAccountGr-detail_rekap" id="lapAccountGr-detail_rekap">
						...
						<div class="one wide field">
							<label style="visibility:hidden">-</label>
							<div class="ui fluid icon green button filter-input" id="lapAccountGr-btnfilter" onclick=filterTransaksi("lapAccountGr") title="Filter">
								<i class="filter icon"></i>
							</div>
						</div>
					</div>
				</div>
				<div class="fifteen wide centered column" id="lapAccountGr-wrap_filter" style="padding-top:0">
				</div>
			</div>
			</form>
		</div>';
	...
	public function filter(){
		...
		$from_date = $this->input->post('lapAccountGr-datefrom');
		$dari_tanggal = $this->input->post('lapAccountGr-datefrom');
		$to_date = $this->input->post('lapAccountGr-dateto');
		$sampai_tanggal = $this->input->post('lapAccountGr-dateto');
		...
		$accountnumber = $this->input->post('lapAccountGr-accountnumber');
		$accountname = $this->mm->get_accountname_by_accountint($accountnumber);
		
		$detail_rekap = $this->input->post('lapAccountGr-detail_rekap');
		
		$site_name = $this->mm->get_site_name();
		
		$idkarat = $this->input->post('lapAccountGr-idkarat');
		if($idkarat == 'All'){
			$karat = $this->mk->get_karat_srt();
		}else{
	...
	public function excel($from_date,$to_date,$accountnumber,$idkarat,$detail_rekap){
		...
		if($detail_rekap == 'R'){
			...
			$sheet->setCellValue('A1', 'Trial Balance Report (Gram), Cabang '.$site_name);
			...
			$sheet->setCellValue('A1', 'Ledger Detail Report (Gram), Cabang '.$site_name);
			$sheet->setCellValue('A2', 'Tanggal '.$tanggal_tulis);
			$sheet->setCellValue('A3', $accountname.', '.$karat_tulis);
		...
		header('Content-Disposition: attachment;filename="EXCEL LAPORAN ACCOUNT (GRAM) '.$site_name.' TANGGAL '.$tanggal_tulis.'.xlsx"');
		//unduh file
