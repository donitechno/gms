...
class LapTahunan extends CI_Controller {
	...
	public function index(){
		...
		<form class="ui form form-javascript" id="lapTahunan-form-filter" action="'.base_url().'index.php/lapTahunan/pdf" method="post">
		<div class="ui grid">
			<div class="ui inverted dimmer" id="lapTahunan-loaderlist">
			...
			<div class="five wide centered column no-print" style="margin-top:15px">
				<div class="fields">
					<div class="ten wide field">
						...
						<input type="text" name="lapTahunan-date" id="lapTahunan-date" readonly>
					</div>
					<div class="six wide field">
						<label style="visibility:hidden">-</label>
						<div class="ui fluid icon green button filter-input" id="lapTahunan-btnfilter" onclick=filterTransaksi("lapTahunan") title="Filter">
	...
	public function pdf($tgl_transaksi){
		...
		foreach($kas_account as $ka){
			...
			$data['view'] .= '<td class="right-aligned">'.number_format($saldo_akhir_kurs[$id_kurs], 3).'</td></tr>';
		...
		$sewa_toko = $this->mm->get_default_account('PST');
		...
		$total_setor_rp = $total_setor_rp + $total_setor;
		
		$setor_dp = $this->mm->get_setor_biaya($sewa_toko,$pidp,$from_date,$to_date);
		...
		$total_setor_rp = $total_setor_rp + $total_setor;
		$total_setor_gr = $total_setor_rp / $harga_emas;
		
		$data['view'] .= '<tr><td></td><td>Setoran Periode Ini</td><td class="right-aligned">'.number_format($total_setor_rp, 0).'</td><td class="right-aligned">'.number_format($total_setor_rp, 0).'</td><td class="right-aligned">'.number_format($total_setor_gr, 3).'</td>';
