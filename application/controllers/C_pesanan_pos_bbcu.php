...
class C_pesanan_pos_bbcu extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('pos/V_pesanan_bbcu');
	}
	...
	public function save(){
		...
		$this->mt->input_main_pesanan_bbcu($id_pesanan,$customer_name,$customer_address,$customer_phone,$ump_val,$tgl_transaksi,$created_by);
	...
	public function save_tambah_ump($id_pesanan){
		...
		$data_main_pesanan = $this->mt->get_pesanan_by_id_bbcu($id_pesanan);
		$ump_main = $data_main_pesanan[0]->ump_val;
		$ump_main = $ump_main + $ump_val;
		
		$this->mt->update_ump_pesanan_bbcu($id_pesanan,$ump_main);
	...
	private function validate_tambah_kurang($id_pesanan){
		...
		$data_pesanan = $this->mt->get_pesanan_by_id_bbcu($id_pesanan);
		$tanggal_pesan = $data_pesanan[0]->trans_date;
	...
	public function filter(){
		...
		$data_filter = $this->mt->get_filter_pesanan_bbcu($from_date,$to_date,$filter_status);
	...
	public function ambil(){
		...
		$data_filter = $this->mt->get_filter_pesanan_ambil_bbcu();
		...
		foreach($data_filter as $d){
			...
			$data['view'] .= '<tr><td class="center aligned">'.$number.'</td><td>'.$d->id_pesanan.'</td><td>'.$tanggal_tulis.'</td><td>'.$d->cust_name.'</td><td>'.$d->cust_address.'</td><td>'.$d->cust_phone.'</td><td class="right aligned">'.number_format($d->ump_val, 0).'</td><td class="center aligned"><div id="btn_search" class="ui mini icon facebook button" title="Ambil Pesanan" onclick=ambilPesananBBCU("'.$d->id_pesanan.'")><i class="hand pointer outline icon"></i></div></td></tr>';
	...
	public function get_kurang_ump($id_pesanan){
		...
		$data['view'] .= '<form class="ui form form-javascript" id="form_tambah_kurang" action="'.base_url().'index.php/C_pesanan_pos_bbcu/save_kurang_ump/'.$id_pesanan.'" method="post"><div class="field"><input type="text" id="ump_date" name="ump_date" readonly></div><div class="field"><input type="text" id="ump_tambah_kurang" name="ump_tambah_kurang" placeholder="Masukkan Nilai UMP" onkeyup=umpToCurrency() style="text-transform:uppercase" autocomplete="off"></div><input type="submit" value="simpan"></form></div><div class="actions"><button id="btn_save_plus_min" class="ui green labeled icon button" onclick=saveTambahKurangUMP()>Simpan	<i class="save icon"></i></button></div>';
		
		$data['success'] = true;
		echo json_encode($data);
	}
	
	public function get_ambil_pesanan_bbcu($id_pesanan){
		$data['view'] = '<i class="close icon"></i><div class="header">Tarik UMP Pesanan '.$id_pesanan.'</div><div class="content"><div class="ui error message" id="error_modal" style="display:none"></div>';
		
		$data['view'] .= '<form class="ui form form-javascript" id="form_ambil_pesanan_bbcu" action="'.base_url().'index.php/C_pesanan_pos_bbcu/save_ambil_pesanan_bbcu/'.$id_pesanan.'" method="post"><div class="field"><input type="text" id="ambil_date_bbcu" name="ump_date" readonly></div></form></div><div class="actions"><button id="btn_save_ambil_bbcu" class="ui green labeled icon button" onclick=saveAmbilPesananBBCU()>Simpan	<i class="save icon"></i></button></div>';
		
		$data['success'] = true;
		echo json_encode($data);
	}
	
	public function save_ambil_pesanan_bbcu($id_pesanan){
		date_default_timezone_set("Asia/Jakarta");
		...
		$tanggal_pesan = $data_pesanan[0]->trans_date;
		$ump_val = $data_pesanan[0]->ump_val;
		
		if($tanggal_pesan > $tanggal_action){
			$data['inputerror'] = '<ul class="list"><li>Tanggal Ambil Tidak Boleh Dibawah Tanggal Input Pesanan!</li></ul>';
			$data['success'] = FALSE;
			echo json_encode($data);
			exit();
		}
		...
		$this->mt->update_ambil_main_pesanan_bbcu($id_pesanan,$status_update,$grosir_use,$total_price,$tanggal_action,$created_by);
		...
		$data['message'] = '<div class="content"><div class="ui icon header center aligned"><i class="check circle outline green icon"></i>Berhasil Tarik UMP Pesanan '.$id_pesanan.'!</div></div>';
