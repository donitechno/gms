...
class C_mail extends CI_Controller {
	...
	public function index(){
		...
		$mail->AddAddress("elga.bandabaru17@gmail.com","Elga Helia"); //tujuan email
		...
		$mail->AddAttachment("report/LAP EXCEL ".$site_name." Tanggal ".$tanggal_aktif.".xlsx");
	...
	public function lap_jual_to_excel($tgl_transaksi){
		$tgl_transaksi = str_replace('%20',' ',$tgl_transaksi);
		$tanggal_transaksi =  $tgl_transaksi;
		...
		while($i < $total_karat){
			...
			$data_jual = $this->mt->get_rekap_karat_excel($karat_id,$before_tgl_transaksi,$tgl_transaksi);
	...
	public function lap_export_data($tgl_transaksi){
		...
		$sheet->setCellValue('A'.$start_data_row, 'gold_mutasi_titip_pengadaan');
		$start_data_row = $start_data_row + 1;
		
		$data_mutasi_titip_pgd = $this->mm->get_mutasi_titip_pgd($tgl_from,$tgl_to);
		...
		$sheet->setCellValue('A'.$start_data_row, 'gold_mutasi_titip_pengadaan_hapus');
		$start_data_row = $start_data_row + 1;
		
		$data_mutasi_titip_pgd_hapus = $this->mm->get_mutasi_titip_pgd_hapus($tgl_from,$tgl_to);
		...
		$sheet->setCellValue('A'.$start_data_row, 'gold_periode');
		$start_data_row = $start_data_row + 1;
		
		$data_periode = $this->mm->get_periode($tgl_from,$tgl_to);
		
		foreach($data_periode as $d){
			$sheet->setCellValue('A'.$start_data_row, $d->id);
			$sheet->setCellValue('B'.$start_data_row, $d->from_date);
			$sheet->setCellValue('C'.$start_data_row, $d->to_date);
			$sheet->setCellValue('D'.$start_data_row, $d->map);
			$sheet->setCellValue('E'.$start_data_row, $d->created_date);
			$sheet->setCellValue('F'.$start_data_row, $d->created_by);
		...
		$sheet->setCellValue('A'.$start_data_row, 'gold_detail_trans_cabang');
		$start_data_row = $start_data_row + 1;
		
		$data_periode = $this->mm->get_detail_trans_cabang($tgl_from,$tgl_to);
		
		foreach($data_periode as $d){
			$sheet->setCellValue('A'.$start_data_row, $d->id);
			$sheet->setCellValue('B'.$start_data_row, $d->transaction_code);
			$sheet->setCellValue('C'.$start_data_row, $d->cabang);
			$sheet->setCellValue('D'.$start_data_row, $d->tipe);
			$sheet->setCellValue('E'.$start_data_row, $d->description);
			$sheet->setCellValue('F'.$start_data_row, $d->id_karat);
			$sheet->setCellValue('G'.$start_data_row, $d->berat_real);
			$sheet->setCellValue('H'.$start_data_row, $d->berat_konversi);
			$sheet->setCellValue('I'.$start_data_row, $d->persentase);
			$sheet->setCellValue('J'.$start_data_row, $d->trans_date);
			$sheet->setCellValue('K'.$start_data_row, $d->created_date);
			$sheet->setCellValue('L'.$start_data_row, $d->created_by);
