...
class LapAccountRp extends CI_Controller {
	...
	public function index(){
		$account = $this->mm->get_account_lain();
		
		$data['view'] = '<div class="ui container fluid">
			<form class="ui form form-javascript" id="lapAccountRp-form-filter" action="'.base_url().'index.php/lapAccountRp/filter/" method="post">
			<div class="ui grid">
				<div class="ui inverted dimmer" id="lapAccountRp-loaderlist">
				...
				<div class="fifteen wide centered column" style="padding-bottom:0">
					<div class="fields">
						<div class="three wide field">
							...
							<input type="text" name="lapAccountRp-datefrom" id="lapAccountRp-datefrom" readonly>
						</div>
						...
						<div class="three wide field">
							...
							<input type="text" name="lapAccountRp-dateto" id="lapAccountRp-dateto" readonly>
						</div>
						<div class="five wide field">
							<label>Account</label>
							<select name="lapAccountRp-filter_account" id="lapAccountRp-filter_account">';
						...
						<div class="three wide field">
							<label style="visibility:hidden">-</label>
							<select name="lapAccountRp-filter_dr" id="lapAccountRp-filter_dr">
						...
						<div class="one wide field">
							<label style="visibility:hidden">-</label>
							<div class="ui fluid icon green button filter-input" id="lapAccountRp-btnfilter" onclick=filterTransaksi("lapAccountRp") title="Filter">
								<i class="filter icon"></i>
							</div>
						</div>
					</div>
				</div>
				<div class="fifteen wide centered column" id="lapAccountRp-wrap_filter" style="padding-top:0">
				</div>
			</div>
			</form>
		</div>';
	...
	public function filter(){
		...
		$from_date = $this->input->post('lapAccountRp-datefrom');
		$dari_tanggal = $from_date;
		$to_date = $this->input->post('lapAccountRp-dateto');
		...
		$filter_account = $this->input->post('lapAccountRp-filter_account');
		$filter_dr = $this->input->post('lapAccountRp-filter_dr');
		
		$kas_account = $this->mm->get_single_coa_rp($filter_account);
		
		if($filter_dr == 'R'){
			$data['view'] = '<div class="sixteen wide centered column full-print" style="padding-top:0px;padding-bottom:0px;text-align:right"><a class="ui purple button" href="'.base_url().'index.php/lapAccountRp/excel/'.$dari_tanggal.'/'.$sampai_tanggal.'/'.$filter_account.'/'.$filter_dr.'"><i class="file excel icon"></i> Export to Excel</a><a class="ui facebook button" href="'.base_url().'index.php/lapAccountRp/pdf/'.$dari_tanggal.'/'.$sampai_tanggal.'/'.$filter_account.'/'.$filter_dr.'" target=_blank"><i class="paperclip icon"></i> Download</a></div><table class="ui celled table" cellspacing="0" width="100%"><thead><tr><th style="width:30px">No</th><th>Account Name</th><th>Saldo Awal</th><th>Tarik</th><th>Setor</th><th>Saldo Akhir</th></tr></thead><tbody>';
	...
	public function pdf($from_date,$to_date,$filter_account,$filter_dr){
        ...
        $pdf->Output("Laporan Account Lain Rupiah.pdf", "I");
	}
	...
	public function excel($from_date,$to_date,$filter_account,$filter_dr){
		...
		if($filter_dr == 'R'){
			...
			$sheet->setCellValue('A1', 'Trial Balance Report, Cabang '.$site_name);
			...
			$sheet->setCellValue('A1', 'Ledger Detail Report, Cabang '.$site_name);
			$sheet->setCellValue('A2', 'Tanggal '.$dari_tanggal.' S/D '.$sampai_tanggal);
			$sheet->setCellValue('A3', $kas_tulis);
