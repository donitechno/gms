...
class C_change_pass extends CI_Controller {
	...
	public function index(){
		$username = $this->session->userdata('gold_username');
		$data['user'] = $this->mk->cek_username_2($username);
		$this->load->view('master/V_change_pass',$data);
	}
	...
	public function change_password(){
		...
		$rep_new_pass = $this->input->post('rep_new_pass');
		
		$random_salt = $this->mk->random_salt();
		$encrypt_pass = $this->mk->encrypt_pass($new_pass, $random_salt);
	...
	private function validate_edit(){
		...
		if($old_pass == ''){
			$data['inputerror'] .= '<li>Password Lama Harus Diisi!</li>';
			$data['success'] = FALSE;
		}
		
		if($new_pass == ''){
			$data['inputerror'] .= '<li>Password Baru Harus Diisi!</li>';
			$data['success'] = FALSE;
		}
		
		if($rep_new_pass == ''){
			$data['inputerror'] .= '<li>Repeat Password Baru Harus Diisi!</li>';
			$data['success'] = FALSE;
		}
		...
		if($old_pass != ''){
			if($this->ml->cek_login($username,$old_pass) == TRUE){
				
			}else{
				$data['inputerror'] .= '<li>Password Lama Salah!</li>';
				$data['success'] = FALSE;
			}
		}
