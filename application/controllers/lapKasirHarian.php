...
class LapKasirHarian extends CI_Controller {
	...
	public function index(){
		...
		<form class="ui form form-javascript" id="lapKasirHarian-form-filter" action="'.base_url().'index.php/lapKasirHarian/filter" method="post">
		<div class="ui grid">
			<div class="ui inverted dimmer" id="lapKasirHarian-loaderlist">
			...
			<div class="five wide centered column no-print" style="margin-top:15px">
				<div class="fields">
					<div class="ten wide field">
						...
						<input type="text" name="lapKasirHarian-date" id="lapKasirHarian-date" readonly>
					</div>
					<div class="six wide field">
						<label style="visibility:hidden">-</label>
						<div class="ui fluid icon green button filter-input" id="lapKasirHarian-btnfilter" onclick=filterTransaksi("lapKasirHarian") title="Filter">
							<i class="filter icon"></i> Filter
						</div>
					</div>
				</div>
			</div>
			<div class="fifteen wide centered column" id="lapKasirHarian-wrap_filter">
			</div>
		</div>
		</form>';
	...
	public function filter(){
		...
		$tgl_transaksi =  $this->input->post('lapKasirHarian-date');
		$tanggal_transaksi =  $this->input->post('lapKasirHarian-date');
	...
	public function pdf($tgl_transaksi){
		...
		$data['view'] .= '<tr><td style="font-weight:bold" colspan="2">Saldo Awal Kasir</td><td></td><td></td><td style="text-align:right;font-weight:bold">'.number_format($saldo_awal, 0).'</td><tr>';
		...
		$trans_tunai = $this->mt->get_mutasi_jual_tunai_detail($tgl_from);
		
		if(count($trans_tunai) != 0){
			foreach($trans_tunai as $tt){
				$saldo_awal = $saldo_awal + $tt->value;
				
				$data['view'] .= '<tr><td colspan="2">Penjualan Tunai</td><td style="text-align:right">'.number_format($tt->value, 0).'</td><td></td><td style="text-align:right">'.number_format($saldo_awal, 0).'</td><tr>';
			}
		}
